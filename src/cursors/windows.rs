...
pub fn load_cursor(
    ...
    hotspot_x: u8,
    hotspot_y: u8,
) -> core::result::Result<windows::Win32::UI::WindowsAndMessaging::HCURSOR, LoadCursorError> {
...
pub fn load_svg_cursor_with_file<C: GetCursorResolution>(
    ...
    let Ok(image_data) = rasterize_svg(svg_data, wanted_size, wanted_size) else {
...
pub fn load_cursor_file(
    file_path: &str,
) -> core::result::Result<windows::Win32::UI::WindowsAndMessaging::HCURSOR, &'static str> {
    unsafe {
        ...
        let Ok(cursor) = LoadCursorFromFileA(PCSTR(filename.as_ptr().cast::<u8>())) else {
            return Err("Windows refused to load. The file used was potentially corrupted");
        };
        if cursor.0 != 0 {
            return Err("Failed to load cursor. Why? No one known, probably not even windows");
        }
...
pub fn create_temp_file(cursor_data: &[u8]) -> std::io::Result<(String, tempfile::NamedTempFile)> {
    ...
    let path_str = file
        .path()
        .to_str()
        .ok_or_else(|| std::io::Error::other("Invalid UTF-8 in temp file path"))?
        .to_string();
...
pub unsafe fn subclass_window(
        ...
        && let Some(actual_cursor) = Some(cursor)
    {
        ...
        ORIGINAL_WNDPROC = Some(std::mem::transmute::<
            isize,
            core::option::Option<
...
/// # Safety
/// Interacts with windows
pub unsafe fn update_cursor(cursor: &windows::Win32::UI::WindowsAndMessaging::HCURSOR) {
